...
class ClientController {
  /**
   * GET /api/clients
   * Return optimized dashboard data
   */
  static async getClients(req, res) {
    try {
      const { search = '', industry = '' } = req.query;
      
      const clients = await ClientService.getAllClients(search, industry);
   ...
   * Seed database with realistic SaaS companies
   */
  static async seedClients(req, res) {
    try {
      const clientsData = [
        {
      ...
      const result = await ClientService.seedClients(clientsData);
